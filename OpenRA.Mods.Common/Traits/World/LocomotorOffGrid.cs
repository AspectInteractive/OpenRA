...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class LocomotorOffGridInfo : TraitInfo
	{
		[Desc("Locomotor ID.")]
		...
		public override object Create(ActorInitializer init) { return new LocomotorOffGrid(init.Self, this); }
	}

	public class LocomotorOffGrid : IWorldLoaded
	{
		...
		public readonly LocomotorOffGridInfo Info;
		...
		readonly LocomotorOffGridInfo.TerrainInfo[] terrainInfos;
		...
		public LocomotorOffGrid(Actor self, LocomotorOffGridInfo info)
		{
			Info = info;
			...
			terrainInfos = new LocomotorOffGridInfo.TerrainInfo[terrainInfo.TerrainTypes.Length];
			for (var i = 0; i < terrainInfos.Length; i++)
				if (!info.TerrainSpeeds.TryGetValue(terrainInfo.TerrainTypes[i].Type, out terrainInfos[i]))
					terrainInfos[i] = LocomotorOffGridInfo.TerrainInfo.Impassable;
